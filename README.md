<p align="center">
  <img src="docs/icon.png" width="120" alt="밀밀 네비바 아이콘">
</p>

<h1 align="center">밀밀 네비바</h1>

<p align="center"><b>Milmil NavBar</b> · 크레마 팔레트를 위한 하단 네비게이션 바</p>
...
[릴리스](../../releases/latest)에서 `milmil-navbar-0.1.3.apk`를 받아 설치합니다.

```bash
# 크레마는 adb 설치를 기기 베리파이어가 막을 수 있어, 최초 1회만:
adb shell settings put global verifier_verify_adb_installs 0

adb install milmil-navbar-0.1.3.apk
```
...
1. **접근성 서비스 켜기** : 뒤로, 스크린샷에 필요합니다. 접근성 설정에서 "밀밀 네비바"를 켭니다. (홈은 접근성이 없어도 기본 홈으로 동작하고, 최근앱·옵션·회전은 크레마 내장 기능이라 접근성과 무관합니다.)
2. **다른 앱 위에 표시 허용** : 바 오버레이 표시에 필요합니다.
3. **시스템 설정 수정 허용** : 밝기 조절에만 필요합니다(밝기 버튼을 쓸 때만). 회전은 크레마 내장 다이얼로그라 이 권한이 필요 없습니다.

### "제한된 설정" 때문에 접근성이 안 켜질 때 (adb)

뒤로, 스크린샷을 쓰려면 접근성 서비스가 필요합니다(그 외 기능은 접근성 없이도 동작). 그런데 사이드로드 앱은 접근성 토글이 "제한된 설정"으로 막혀 있을 수 있고, 크레마에는 이 차단을 푸는 "제한된 설정 허용" 메뉴가 없습니다. 그럴 때 아래 명령으로 차단을 해제한 뒤, 설정 → 접근성에서 "밀밀 네비바"를 직접 켜세요.

먼저 기기에서 USB 디버깅을 켜야 합니다. (설정 → 기기 정보에서 빌드번호를 7번 눌러 개발자 옵션을 켠 뒤, 개발자 옵션에서 USB 디버깅 활성화)
...
- **바 열기** : 화면 하단 가장자리에서 위로 스와이프합니다. 탭만으로는 열리지 않습니다(실수로 열리는 것을 막기 위함).
